...
export default function Simulator() {
  ...
  const handleModelLoad = () => {
    ...
    // Increased by 3 seconds as requested (1500 + 3000 = 4500)
    // We use a ref to prevent double-calls or race conditions if desired, 
    // but the state setter is safe.
    setTimeout(() => setIsLoading(false), 4500);
  };

  // Fail-safe: If model doesn't load in 15s, remove loader (it might have crashed or network is slow)
  useEffect(() => {
    const timer = setTimeout(() => {
      if (isLoading) {
        console.warn("Simulator loader timed out - forcing removal.");
        setIsLoading(false);
      }
    }, 15000);
      ...
      <div className="sim-viewport-wrapper" style={{ background: '#0d1117' }}>
        {hasError ? (
          ...
          >
            ...
            <OrbitControls makeDefault enabled={introFinished} maxDistance={12} minDistance={2} />
          </Canvas>
        )}
      </div>
      ...
      <div className="slider-header">
        ...
        <div className="slider-value-group">
          <input
            ...
            step={step}
            onChange={(e) => {
              // Allow typing any number, clamp only on blur/enter to allow typing "10" when min is "5"
              onChange(Number(e.target.value));
            }}
            onBlur={(e) => {
              // Clamp on exit to ensure valid simulation state
              const val = Math.min(Math.max(Number(e.target.value), min), max);
              if (val !== Number(e.target.value)) onChange(val);
            }}
            onKeyDown={(e) => {
              if (e.key === 'Enter') {
                const val = Math.min(Math.max(Number(e.currentTarget.value), min), max);
                onChange(val);
